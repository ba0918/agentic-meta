...
| CA-U001 | unsafe | WARN | REPORT_ONLY | Pure function | Wording that permits skipping confirmation or performing a destructive operation. The line is quoted when it comes from an instruction file, and held back when it comes from a memory, which is named by its place and the kind of wording instead |
